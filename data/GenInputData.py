...
with open(BANK_FILE, "w", encoding="latin-1") as bank_file:
    for reg_number, bank_name, bank_address, phone_number, email in bank_data:
        bank_record = (
            ...
            f"{email:<31}"             # Emailadresse (30 tegn)
        )
...
with open(TRANSACTION_FILE, "w",  encoding="latin-1") as file:
    for i in range(1, NUM_CUSTOMERS + 1):  # Loop over kunder
        ...
        for _ in range(num_transactions):  # Loop over kundens transaktioner
            ...
            record = (
                ...
                f"{fødselsdato:<10}"     # Fødselsdato
                f"{konto_nummer:<12}"    # Kontonummer
